...
public class AuthValidatorHelper<V, R, B> {
     ...
     */
    public static <V, R, B> B isValid(AuthValidator<V, R> authValidator, String id, V verifier, R request, B success, B failure) {
        try {
            if (authValidator.isValid(id, verifier, request)) {
                LOG.trace("Jive protocol and OAuth signature verification passed for id:{} verifier:{} request:{}",
                    id, verifier, request);
                return success;
            } else {
                LOG.warn("Jive protocol signature passed but OAuth signature did not pass for id:{} verifier:{} request:{}",
                    id, verifier, request);
                return failure;
            }
        } catch (AuthValidationException ex) {
            LOG.warn("Jive protocol signature did not pass, OAuth signature not attempted for id:{} verifier:{} request:{} protocol error:{}",
                new Object[]{id, verifier, request, ex.toString()});
            return failure;
        }
    }
